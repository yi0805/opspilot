...
export function QuestionForm({ question, loading, onQuestionChange, onSubmit }: QuestionFormProps) {
  ...
  return <form className="question-form" onSubmit={submit}>
    ...
    <textarea id="business-question" value={question} maxLength={500} disabled={loading} onChange={(event) => onQuestionChange(event.target.value)} placeholder="Compare FW-100 sales and inventory. Is there a replenishment risk and what should we do?" rows={4} />
