...
function DataDisplay({ data }: { data: JsonValue }) {
  ...
  if (Array.isArray(data) && data.every(isRecord)) {
    ...
    const fields = [...new Set(rows.flatMap((row) => Object.keys(row)))].filter((field) => rows.every((row) => field in row && isScalar(row[field])))
    if (fields.length > 0) return <div className="table-scroll"><table><thead><tr>{fields.map((field) => <th key={field}>{readableLabel(field)}</th>)}</tr></thead><tbody>{rows.map((row, index) => <tr key={index}>{fields.map((field) => <td key={field}>{displayValue(row[field])}</td>)}</tr>)}</tbody></table></div>
  }
