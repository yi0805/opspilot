...
test('shows a usable error for backend HTTP failures', async () => { installFetch(vi.fn().mockResolvedValue(response({ detail: 'OPENAI_API_KEY is not configured.' }, false, 503))); render(<App />); fireEvent.change(screen.getByLabelText('Business question'), { target: { value: 'Check FW-100' } }); fireEvent.click(screen.getByRole('button', { name: 'Analyze' })); await waitFor(() => expect(screen.getByRole('alert')).toHaveTextContent('OpsPilot could not complete the request.')); expect(screen.getByRole('alert')).toHaveTextContent('OPENAI_API_KEY is not configured.'); expect(screen.getByRole('button', { name: 'Analyze' })).toBeEnabled() })
